...
let appState = {
    ...
    testsCompleted: false // Track if all tests are finished
};
...
function connectWebSocket() {
    ...
    appState.ws.onopen = function(event) {
...
function closeWebSocket() {
    if (appState.ws && appState.ws.readyState === WebSocket.OPEN) {
        ...
        appState.ws.close(1000, 'Tests completed');
        appState.ws = null;
    }
}
...
function handleRealTimeUpdate(data) {
    ...
    // Check if all tests are completed (no running tests and we have results)
    const allTestsCompleted = hasResults && appState.summary.running === 0 && appState.summary.total > 0;
    ...
    if (hasResults) {
        ...
        // Close WebSocket if all tests are completed
        if (allTestsCompleted && !appState.testsCompleted) {
            appState.testsCompleted = true;
            console.log('🏁 All tests completed, closing WebSocket connection');
            closeWebSocket();
            document.getElementById('last-updated').textContent = `Tests completed: ${new Date().toLocaleString()}`;
        }
    } else {
        // No results yet
        appState.results = [];
    ...
    // Update last updated time (only if tests aren't completed)
    if (!appState.testsCompleted) {
    ...
    console.log('📊 Dashboard updated via WebSocket (smart):', {
        ...
        testsCompleted: appState.testsCompleted,
...
function resetTestState() {
    console.log('🔄 Resetting test state for new run');
    ...
    document.getElementById('last-updated').textContent = 'Starting new test run...';
...
async function startTests() {
    ...
    appState.testsCompleted = false; // Reset completion flag
    btn.disabled = true;
    ...
    if (!appState.ws || appState.ws.readyState !== WebSocket.OPEN) {
        console.log('🔌 Reconnecting WebSocket for new test run');
        connectWebSocket();
    }
...
async function refreshTests() {
    ...
    // Reconnect WebSocket if it was closed
    if (!appState.ws || appState.ws.readyState !== WebSocket.OPEN) {
        console.log('🔌 Reconnecting WebSocket for refresh');
        connectWebSocket();
    }
